...
int SOFADataOld::initSofaData(const char* filePath, int sampleRate)
{
...
int SOFADataOld::getLengthOfHRIR(){
    return lengthOfHRIR;
}


fftwf_complex* SOFADataOld::getHRTFforAngle(float elevation, float azimuth, float radius){
...
fftwf_complex* SOFADataOld::getMinPhaseHRTFforAngle(float elevation, float azimuth, float radius){
...
float* SOFADataOld::getHRIRForAngle(float elevation, float azimuth, float radius){
    int best_id = 0;
...
ITDStruct SOFADataOld::getITDForAngle(float elevation, float azimuth, float radius){
    int best_id = 0;
...
int SOFADataOld::loadSofaFile(const char* filePath, int hostSampleRate){
...
sofaMetadataStruct SOFADataOld::getMetadata(){
    return sofaMetadata;
}
...
void SOFADataOld::errorHandling(int status) {
    String ErrorMessage;
    switch (status) {
        case ERR_MEM_ALLOC:
...
//
//
void SOFADataOld::createPassThrough_FIR(int _sampleRate){
...
String SOFADataOld::getSOFAGlobalAttribute(const char* attribute_ID, int ncid){
    size_t att_length = 0;
