...
public:
    ...
    void prepareToPlay(SOFAData *sD, int _sampleRate, int _filterBlockSize);
    
//    int initWithSofaData(SOFAData* sD, int _sampleRate, int blockSize);
    
private:
    void processNearfield(soundSourceData data, const float* inBuffer, float* outBuffer_L, float* outBuffer_R);
    void processFarfield(soundSourceData data, const float* inBuffer, float* outBuffer_L, float* outBuffer_R);
