...
int EarlyReflection::initWithSofaData(SOFAData *sD, int _sampleRate, float angle_az, float angle_el){
    ...
    fftwf_complex* _hrtf = sD->getHRTFforAngle(elevation, azimuth, 1.0, hrtf_type_original);
...
void EarlyReflection::prepareToPlay(){
    ...
    filter.prepareToPlay(sofaData->getHRTFforAngle(0.0, 0.0, 1.0, hrtf_type_original));
